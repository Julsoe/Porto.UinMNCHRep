...
class InterpretTaskActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // assigning Constants' scan task list to the variable
        ...
        if (ContextCompat.checkSelfPermission(this@InterpretTaskActivity,
                Manifest.permission.ACCESS_FINE_LOCATION) !==
            PackageManager.PERMISSION_GRANTED) {
            if (ActivityCompat.shouldShowRequestPermissionRationale(this@InterpretTaskActivity,
                    Manifest.permission.CAMERA)) {
                ActivityCompat.requestPermissions(this@InterpretTaskActivity,
    ...
    private fun setupTaskView() {
        ...
        // making cipher element invisible for this particular task
        cipherLayout?.visibility = View.INVISIBLE
        ...
        if (Constants.isNetworkAvailable(this)) {
            ivStoredImg?.let {
                ...
                Glide.with(this@InterpretTaskActivity)
                    .load(url)
                    .into(it)
            }
        }
        ...
        when (requestCode) {
            1 -> {
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    if ((ContextCompat.checkSelfPermission(this@InterpretTaskActivity, Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) &&
                        (ContextCompat.checkSelfPermission(this@InterpretTaskActivity, Manifest.permission.INTERNET) == PackageManager.PERMISSION_GRANTED)
                    ) {
                        // Both camera and internet permissions are granted
                        Log.i("Permission status", "Permission granted")

                        // onClick to launch camera and take a photo
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == Constants.REQUEST_CODE && data != null) {
            ...
            if (capturedImg != null) {
                ...
                if (storedImageBitmap != null) {
                    ...
                    Toast.makeText(this, "Unable to load image from API", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
    ...
    private fun displayResultDialog() {
        ...
        btnNextTask.setOnClickListener {
            resultDialog.dismiss()
            val intent = Intent(this, CodeTaskActivity::class.java)
            startActivity(intent)
        }
